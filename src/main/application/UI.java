...
public class UI {
    ...
    private final Color pause_equipment = new Color(102, 87, 53, 115);
    ...
    private final BufferedImage dialogue_finish;
    private final BufferedImage zTargetLock;
    private final BufferedImage zTargetLocked;
    private final BufferedImage flippers;
    private final BufferedImage heart_4;
    private final BufferedImage heart_3;
    private final BufferedImage heart_2;
    private final BufferedImage heart_1;
    private final BufferedImage heart_0;
    private final BufferedImage rupee;
    private final BufferedImage key;
    private final BufferedImage boss_key;
    ...
    // MUSIC
    private final BufferedImage music_sheet;
    private final BufferedImage note_a;
    private final BufferedImage note_d;
    private final BufferedImage note_r;
    private final BufferedImage note_l;
    private final BufferedImage note_u;
    private final HashMap<String, String> music_learned;
    ...
    private void drawTitleScreen() {
        ...
        try {
            image = ImageIO.read(Objects.requireNonNull(
                    getClass().getResourceAsStream("/misc/MENU_TITLE.png")
            ));
        }
        catch (IOException e) {
    ...
    private void menu_newGame() {
        ...
        String keyboard = (gp.keyH.capital) ? "QWERTYUIOPASDFGHJKLZXCVBNM" : "qwertyuiopasdfghjklzxcvbnm";
    ...
    private void menu_loadGame() {
        ...
        if (commandNum == 0) {
            drawSubWindow(x + 40, y, width, height, Color.GREEN);
        }
        else {
            drawSubWindow(x + 40, y, width, height);
        }

        y += (int) (gp.tileSize * 1.7);
        if (commandNum == 1) {
            drawSubWindow(x + 40, y, width, height, Color.GREEN);
        }
        else {
...
//		if (gp.currentArea == gp.dungeon) drawKeys();		
    ...
    private void drawHealth() {
        ...
        // QUARTER HEARTS
            ...
            while (i < gp.player.life) {
                ...
                if (i + 4 <= gp.player.life) {
                    heart = heart_4;
                    c = 4;
                }
                else if (i + 3 == gp.player.life) {
                    heart = heart_3;
                    c = 3;
                }
                else if (i + 2 == gp.player.life) {
                    heart = heart_2;
                    c = 2;
                }
                else if (i + 1 == gp.player.life) {
                    heart = heart_1;
                    c = 1;
                }
    ...
    private void drawDebug() {
        ...
        String timeOfDay = switch (gp.eManager.lighting.dayState) {
            case 0 -> "DAY";
            case 1 -> "DUSK";
            case 2 -> "NIGHT";
            case 3 -> "DAWN";
    ...
    private void pause_settings() {
        ...
        switch (subState) {
            case 0:
                pause_settings_top(frameX, frameY);
                break;
            case 1:
                pause_settings_fullscreenNotif(frameX);
                break;
            case 2:
                pause_settings_controls(frameX, frameY);
                break;
            case 3:
                pause_settings_saveGameSlot(frameX);
                break;
            case 4:
                pause_settings_saveGameConfirm(frameX);
                break;
            case 5:
                pause_settings_loadGameSlot(frameX);
                break;
            case 6:
                pause_settings_loadGameConfirm(frameX);
                break;
            case 7:
                pause_settings_quitGameConfirm(frameX);
                break;
        }
    }
    ...
    private void pause_settings_top(int frameX, int frameY) {
        ...
        if (textSpeed == 0) {
            g2.drawString("FAST", textX, textY);
        }
        else if (textSpeed == 1) {
            g2.drawString("MEDIUM", textX, textY);
        }
        else if (textSpeed == 2) {
            g2.drawString("SLOW", textX, textY);
        }
    ...
    private void pause_settings_fullscreenNotif(int frameX) {
    ...
    private void pause_settings_controls(int frameX, int frameY) {
        ...
        textY = frameY + (gp.tileSize * 2 + 10);
        g2.drawString("ACTION", textX, textY);
        textY += gp.tileSize;
        g2.drawString("ATTACK", textX, textY);
        textY += gp.tileSize;
        g2.drawString("ITEM", textX, textY);
        textY += gp.tileSize;
        g2.drawString("TARGET", textX, textY);
        textY += gp.tileSize;
        g2.drawString("SHIELD", textX, textY);
        textY += gp.tileSize;
        g2.drawString("CYCLE ITEMS", textX, textY);
        textY += gp.tileSize;
        g2.drawString("MINI-MAP", textX, textY);

        // VALUES
        textX = frameX + (gp.tileSize * 8);
        textY = frameY + (gp.tileSize * 2 + 10);
        g2.drawString(KeyEvent.getKeyText(gp.btn_A), textX, textY);
        textY += gp.tileSize;
        g2.drawString(KeyEvent.getKeyText(gp.btn_B), textX, textY);
        textY += gp.tileSize;
        g2.drawString(KeyEvent.getKeyText(gp.btn_X), textX, textY);
        textY += gp.tileSize;
        g2.drawString(KeyEvent.getKeyText(gp.btn_L), textX, textY);
        textY += gp.tileSize;
        g2.drawString(KeyEvent.getKeyText(gp.btn_R), textX, textY);
        textY += gp.tileSize;
        g2.drawString(KeyEvent.getKeyText(gp.btn_Z), textX, textY);
        textY += gp.tileSize;
        g2.drawString(KeyEvent.getKeyText(gp.btn_DDOWN), textX, textY);
        textY += gp.tileSize;
    ...
    private void pause_settings_saveGameSlot(int frameX) {
    ...
    private void pause_settings_saveGameConfirm(int frameX) {
    ...
    private void pause_settings_quitGameConfirm(int frameX) {
    ...
    private void pause_settings_loadGameSlot(int frameX) {
    ...
    private void pause_settings_loadGameConfirm(int frameX) {
    ...
    public void drawDialogueScreen(boolean skip) {
        ...
        // NPC HAS NO MORE DIALOGUE
            ...
            else if (npc.hasItemToGive) {
                gp.player.canObtainItem(npc.inventory.getFirst());
            }
            else {
    ...
    private void drawMusic() {
        ...
        if (!gp.player.music_notes.isEmpty()) {
    ...
    private void checkSong() {
        ...
        String song_played = String.join("", gp.player.music_notes);
    ...
    private void playNoteD() {
        gp.playSE(9, 1);
    }

    private void playNoteR() {
        gp.playSE(9, 2);
    }

    private void playNoteL() {
        gp.playSE(9, 3);
    }

    private void playNoteU() {
        gp.playSE(9, 4);
    }
    ...
    private BufferedImage rotateImage(BufferedImage in, int degrees) {
        ...
        AffineTransform rotation = AffineTransform.getRotateInstance(
                Math.toRadians(degrees), (double) in.getWidth() / 2, (double) in.getHeight() / 2
        );
