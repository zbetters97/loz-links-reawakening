...
public class Entity {
    ...
    protected String getPlayerDirection() {
        ...
        String playerDirection = switch (gp.player.direction) {
    ...
    public void draw(Graphics2D g2) {
        ...
        if (inFrame() && drawing) {
            ...
            if (action == Action.JUMPING) {
                drawShadow(g2, 5, 50, tempScreenY + 40);
            }
            if (thrown) {
                drawShadow(g2, 5, 70, tWorldY - gp.player.worldY + gp.player.screenY + 40);
            }
            if (captured) {
                changeAlpha(g2, 0.7f);
            }
