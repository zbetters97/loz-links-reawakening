...
public class EventHandler {
    ...
    public void checkEvent() {
        ...
        if (canTouchEvent) {
            ...
            // FAIRY POOL
            if (hit(3, 29, 40, List.of("right"), false)) {
                healingPool();
            }
            // SHOPKEEPER
            else if (hit(1, 10, 9, "right", 5, Arrays.asList("up", "upleft", "upright"))) {
                speak(gp.npc[1][0]);
            }
            // TELEPORT SPOTS
            else if (hit(0, 10, 39, true)) {
                teleport(1, 12, 13, gp.shop, 1, "up"); // SHOP ENTRANCE
            }
            else if (hit(1, 12, 13, true)) {
                teleport(0, 10, 39, gp.outside, 2, "down"); // SHOP EXIT
            }
            ...
            else if (hit(2, 40, 93, true)) {
                teleport(0, 12, 12, gp.outside, 2, "down"); // DUNGEON EXIT
            }
            else if (hit(2, 70, 49, true)) {
                teleport(3, 18, 40, gp.dungeon, 3, "right"); // DUNGEON F2
            }
            else if (hit(3, 18, 40, true)) {
                teleport(2, 70, 49, gp.dungeon, 2, "down"); // DUNGEON F1
            }
            ...
            if (!Progress.enemy_room_1_1) {
                if (hit(2, 33, 79, "down", 3)) {
                    ...
                    spawnEnemies(
                            Arrays.asList(
                                    new EMY_ChuChu_Red(gp, 29, 81),
                                    new EMY_ChuChu_Green(gp, 29, 78)
                            ),
                            new int[]{35, 29},
                            new int[]{80, 76},
                            new String[]{"left", "down"},
            ...
            if (!Progress.enemy_room_1_2) {
                if (hit(2, 23, 63, "down", 3)) {
                    ...
                    spawnEnemies(
                            Arrays.asList(
                                    new EMY_ChuChu_Red(gp, 16, 64),
                                    new EMY_ChuChu_Red(gp, 18, 64)
                            ),
                            new int[]{25},
                            new int[]{64},
                            new String[]{"left"},
            ...
            if (!Progress.enemy_room_1_3) {
                if (hit(2, 58, 65, "right", 3)) {
                    ...
                    spawnEnemies(
                            Arrays.asList(
                                    new EMY_Wizzrobe(gp, 57, 62),
                                    new EMY_Buzzblob(gp, 62, 62)
                            ),
                            new int[]{59, 59},
                            new int[]{67, 60},
                            new String[]{"up", "down"},
            ...
            for (Entity entity : enemyList) {
                ...
                for (int c = 0; c < gp.enemy[1].length; c++) {
                    ...
                    if (gp.enemy[gp.currentMap][c] == null) {
                        gp.enemy[gp.currentMap][c] = entity;
    ...
    private void boss(int num) {
        ...
        if (!gp.bossBattleOn) {
			if (num == 1) {
				gp.csManager.scene = gp.csManager.boss_1;
			}
